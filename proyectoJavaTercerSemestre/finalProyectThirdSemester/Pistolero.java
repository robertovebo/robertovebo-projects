...
public class Pistolero extends Personaje {
    ...
    public Pistolero(String nombre, int vidaMaxima, int vidaActual, int poderDeAtaque, int poderDeHabilidad, String arma, int nivel,
                     int experiencia) {
    ...
    public String habilidad(Personaje personajeContrincante) {
        personajeContrincante.setVidaActual(personajeContrincante.getVidaActual() - getPoderDeAtaque() + 2);
        setExperiencia(getExperiencia() + 4);
        return "\nSe le restó " + (getPoderDeAtaque() * 3) + " puntos de vida al contrincante\n" + subeNivel();
    }
    ...
    public String descripcionHabilidad() {
        return "------------------------------------------------------------------------------------------\n" +
               "| - Tiro de práctica (Habilidad): Este ataque te bonifica +4 puntos de experiencia, pero |\n" +
               "|   inflije 2 puntos menos de poder de ataque -                                          |\n" +
               "------------------------------------------------------------------------------------------\n";
    }
